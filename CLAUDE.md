...
| `parser.py` | PGS segment payload parsers (`parse_pcs`, `parse_pds`, `parse_ods`), RLE decoder, `ds_has_content()` |
...
| `--tracks all` (no limit) | `libpgs stream <file> -t <id>` per track — full extraction, no limit. |
