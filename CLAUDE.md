...
| `--first N` or custom interactive count | `libpgs stream <file> -t <id>` — pipe closed once N display sets collected. Reuses analysis cache if it already has enough content. |
| `--tracks all` (no limit), cues present | `libpgs stream <file> -t <id>` per track — libpgs seeks directly via MKV Cues, no re-reading. |
| `--tracks all` (no limit), no cues | Single `libpgs stream <file> -t id1,id2,...` pass. A reader thread demuxes into per-track queues consumed by concurrent renderer threads. Avoids re-reading the file from the start for each track. |
